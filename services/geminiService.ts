...
export const generateCourseContent = async (topic: string): Promise<CourseContent> => {
  try {
    ...
    3.  **Developer Agent**: As a developer, you will provide clean, well-commented code examples where relevant to the topic. You will also suggest descriptive prompts for images where they would be illustrative.
...
export const generateImage = async (prompt: string): Promise<string> => {
    try {
      ...
      const response = await ai.models.generateImages({
        ...
        prompt: `A high-quality, professional, educational illustration for a web course. Style: clean, modern, slightly abstract vector art. Subject: ${prompt}`,
        config: {
          numberOfImages: 1,
      ...
      throw new Error(`Failed to generate image from AI model for prompt: "${prompt}"`);
    }
  };